...
describe("AI overlay declarative binding contract", () => {
	it("documents 3-layer data utilization strategy in system prompt", () => {
		...
		// WebCG-K 자체 모션 런타임 선언
		expect(OVERLAY_SYSTEM_PROMPT).toContain("data-motion");
		expect(OVERLAY_SYSTEM_PROMPT).toContain("data-motion-text");
		expect(OVERLAY_SYSTEM_PROMPT).toContain("webcgk.motion");
