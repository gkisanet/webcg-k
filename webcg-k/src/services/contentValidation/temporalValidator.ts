...
import type { CuesheetCheckContext } from "../cuesheetCheckService";
import type { ContentIssue } from "./index";

// ─── 상대 시간 표현 사전 ──────────────────────────────────────────

const RELATIVE_TIME_WORDS = [
	{ word: "오늘", desc: "방송일 기준 '오늘'", offsetDays: 0 },
	{ word: "어제", desc: "방송일 기준 '어제'", offsetDays: -1 },
	{ word: "내일", desc: "방송일 기준 '내일'", offsetDays: 1 },
	{ word: "그저께", desc: "방송일 기준 '그저께'", offsetDays: -2 },
	{ word: "모레", desc: "방송일 기준 '모레'", offsetDays: 2 },
...
export function validateTemporal(
	...
	const reusable = checkContext.reusePolicy === "reusable";
		...
		if (text.includes(entry.word)) {
			const absoluteDate =
				typeof entry.offsetDays === "number"
					? formatKoreanDate(
							addDays(checkContext.programDate, entry.offsetDays),
						)
					: null;
			const suggestion = absoluteDate
				? `${absoluteDate}처럼 절대 날짜 병기`
				: "절대 날짜 또는 기준일 병기";

			issues.push({
				type: "temporal",
				severity: reusable ? "warning" : "info",
				field: fieldKey,
				original: entry.word,
				suggestion,
				message: reusable
					? `📅 "${entry.word}" — ${entry.desc}. 재활용 런다운에서는 ${suggestion}가 필요합니다. 기준일: ${checkContext.programDate}`
					: `📅 "${entry.word}" — ${entry.desc}. 기준일: ${checkContext.programDate}${absoluteDate ? `, 해석: ${absoluteDate}` : ""}`,
			});
		}
	}
