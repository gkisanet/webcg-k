...
context는 CG 화면에 직접 표시되지 않지만, PD가 콘텐츠를 검토할 때
...
\`\`\`json
{
  ...
  "scenes": [
    ...
    },
    {
      "order": 2,
      "trigger": "핵심 통계 발표",
      "graphic_intent": "충격적인 수치를 시각적으로 강조하여 시청자 주의를 환기",
      "duration": 8,
      "_design_rationale": "통계 수치를 fullscreen으로 크게 띄워 임팩트를 준다",
      "text_slots": [
        {
          "semantic_role": "label",
          "value": "충격",
...
4. **모든 value는 실제 데이터**: 대본/자료에서 추출한 실제 텍스트로 채우세요.
5. 한 번의 응답에 **최대 8개의 핵심 장면**만 구성하세요.
6. **Zone 분배**: 90%는 bottom_bar. 특별히 강조할 때만 center나 fullscreen 사용.
7. 반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이 순수 JSON만.`;
}
...
- 절대 요약하거나 건너뛰지 말고, 모든 중요한 정보를 장면으로 구성할 것`;
...
function validateTextSlot(slot: Record<string, unknown>, sceneIdx: number, slotIdx: number, warnings: string[]): TextSlot | null {
  const role = slot.semantic_role;
  const value = slot.value;

  if (typeof role !== "string" || !isValidSemanticRole(role)) {
    warnings.push(`Scene ${sceneIdx + 1} Slot ${slotIdx + 1}: semantic_role이 유효하지 않습니다.`);
    return null;
  }
  if (typeof value !== "string" || !value.trim()) {
    warnings.push(`Scene ${sceneIdx + 1} Slot ${slotIdx + 1}: value가 비어 있습니다.`);
    return null;
  }
  ...
  return {
    semantic_role: role,
    value: value.trim(),
    context: typeof slot.context === "string" && slot.context.trim() ? slot.context.trim() : undefined,
...
function validateAndBuildCuesheet(parsed: Record<string, unknown>, warnings: string[]): AiCuesheet | null {
  ...
  for (let i = 0; i < parsed.scenes.length; i++) {
    ...
    for (let j = 0; j < textSlotsRaw.length; j++) {
      const slot = validateTextSlot(textSlotsRaw[j], i, j, warnings);
    ...
    if (textSlots.length === 0) {
      ...
      if (sscene && typeof sscene === "object") {
        textSlots = convertV3SemanticNodesToSlots(sscene, i);
        if (textSlots.length > 0) {
          usedV3Fallback = true;
...
function extractScenesArray(json: string, warnings: string[]): SceneContent[] | null {
  ...
  for (let j = 0; j < items.length; j++) {
    ...
    for (let k = 0; k < textSlotsRaw.length; k++) {
      const slot = validateTextSlot(textSlotsRaw[k], j, k, warnings);
      if (slot) textSlots.push(slot);
    }
    ...
    if (textSlots.length === 0) {
      const sscene = s.semantic_scene as Record<string, unknown> | undefined;
      if (sscene && typeof sscene === "object") {
        textSlots = convertV3SemanticNodesToSlots(sscene, j);
        if (textSlots.length > 0) {
...
입력된 텍스트 값을 **절대 수정, 축약, 변형하지 마세요.** 주어진 그대로 사용하세요.
템플릿이나 슬롯을 만들지 마세요. 최종 그래픽을 생성하세요.
...
OBS에서 CG 그래픽을 영상 위에 오버레이로 합성하기 위해 필수입니다.
...
function buildGraphicUserPrompt(
  scene: SceneContent,
  programTitle: string,
  options?: {
    themeTokens?: ThemeTokens | null;
...
- 지속 시간: ${scene.duration}초`);
  ...
  for (const slot of scene.text_slots) {
    parts.push(`- [${slot.semantic_role}] "${slot.value}" — 중요도 ${slot.importance}, 존 ${slot.zone_hint}, 스타일 ${slot.style_hint}`);
  }
...
텍스트는 절대 수정하지 말고 주어진 그대로 사용하세요.`);

  return parts.join("\n");
}

function parseGraphicResponse(raw: string): SceneGraphicResult {
  let cleaned = raw.trim();
  const codeBlockMatch = cleaned.match(/```(?:json)?\s*([\s\S]*?)(?:```|$)/);
