...
## 3.1 선언형 데이터 바인딩 (Declarative Data Binding) — 우선 사용 필수
데이터를 DOM에 반영할 때 \`document.getElementById().textContent = data.xxx\` 같은 JS DOM 조작 대신, **HTML 속성(data-cg-*)으로 바인딩하세요.** 플랫폼 런타임이 자동으로 데이터를 DOM에 주입합니다.
...
### JS가 필요한 경우 (data-cg-*로 불충분할 때만)
- 타이머: \`webcgk.computeTimerRemaining(data)\` + \`requestAnimationFrame\` 렌더링 루프
- 복잡한 계산: 점수 차이 계산, 조건부 텍스트 조합 등
- 애니메이션 상태머신: 씬 전환, 캐릭터 액션, 복잡한 e스포츠 연출
- \`webcgk.sendToParent()\` 액션 전송

### 규칙
1. 간단한 텍스트/이미지/boolean 바인딩은 \`data-cg-*\`를 사용하고 JS DOM 조작을 쓰지 마세요.
2. \`data-cg-*\`로 해결 불가능한 로직만 \`webcgk.onData()\` JS를 사용하세요.
3. JS가 필요 없는 오버레이의 \`js\` 필드는 빈 문자열 \`""\`로 설정하세요.
4. \`dashboard_schema.properties\`의 모든 key는 HTML의 \`data-cg-bind\`, \`data-cg-class\`, \`data-cg-if\` 또는 고급 JS 로직에서 한 번 이상 사용되어야 합니다.
...
export function validateOverlayBindings(result: OverlayCodeResult): OverlayBindingValidation {
	...
	for (const key of schemaKeys) {
		if (!bindKeys.has(key) && !jsKeys.has(key)) {
			missingBindings.push(key);
			warnings.push(`스키마 키 "${key}"가 HTML(data-cg-*) 또는 JS에서 사용되지 않습니다.`);
		}
	...
	return {
		ok: missingBindings.length === 0 && orphanBindings.length === 0,
		warnings,
		missingBindings,
		orphanBindings,
		missingDefaults,
