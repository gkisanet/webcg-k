...
webcgk.onData(function(data) {
  // data-cg-*로 해결 불가능한 타이머, 계산, 상태머신, 액션만 작성.
});
...
## 3.1 선언형 데이터 바인딩 (Declarative Data Binding) & 창의적 자율성 (Aesthetic Autonomy)
데이터를 DOM에 기본 매핑할 때는 **HTML 속성(data-cg-*)을 활용하는 것이 편리**합니다. 플랫폼 런타임이 자동으로 데이터를 DOM에 주입해 주기 때문입니다.

| 디렉티브 | 역할 | 예시 |
|----------|------|------|
| \`data-cg-bind="fieldKey"\` | textContent 바인딩 | \`<span data-cg-bind="homeName">HOME</span>\` |
| \`data-cg-bind="src:fieldKey"\` | 속성 바인딩(src, alt 등 안전 속성) | \`<img data-cg-bind="src:teamLogo">\` |
| \`data-cg-class="className:fieldKey"\` | 조건부 CSS 클래스 토글 | \`<div data-cg-class="danger:isWarning">\` |
| \`data-cg-if="fieldKey"\` | 조건부 표시/숨김(display 토글) | \`<div data-cg-if="isLive">LIVE</div>\` |

### 🎨 미학적 애니메이션 & 고급 연출 (Creative Aesthetics is Key!)
- **방송 그래픽의 생명은 생동감과 화려함**입니다. 단순 데이터 매핑을 넘어선 화려하고 프리미엄한 디자인을 위해, **GSAP 애니메이션, CSS variables 테마 바인딩, WebGL2, 복잡한 SVG 애니메이션 상태머신을 적극적으로 설계하십시오.**
- 복잡한 씬 전환, 타이머 카운트다운, 스무스한 레이아웃 전이(Transition) 등이 필요한 경우, 선언형 바인딩에 얽매이지 말고 **\`webcgk.onData(function(data) { ... })\` 내에서 자유롭게 JS 비즈니스 로직과 DOM 애니메이션 라이브러리를 동원하여 창의적인 무대를 만드세요.**
- 주조색, 강조색, 텍스트 색상 등 디자인 스키마 변수들(\`primaryColor\`, \`accentColor\`, \`textColor\`)은 CSS 내에서 \`var(--primaryColor)\` 와 같이 **CSS Variables 테마 변수로 직접 연결하여 극도로 우아한 스타일링 시스템**을 완성하십시오.

### 규칙
1. 단순 텍스트/이미지/boolean 매핑은 \`data-cg-*\`를 사용하면 편리합니다.
2. 하지만 **비주얼 연출, 애니메이션, 실시간 타이머, e스포츠 스타일 연출** 등 예술적 기량이 필요한 부분은 **적극적으로 JS 비즈니스 로직과 CSS 변수 시스템을 설계**하세요.
3. \`dashboard_schema.properties\`에 정의된 모든 키는 HTML 바인딩, JS 로직, 혹은 **CSS 테마 변수 중 하나 이상에서 활용**되기만 하면 정합성 검사를 정상 통과합니다.

## 4. 대시보드 스키마 (Dashboard Schema)
제어 UI 자동 생성을 위해 조작 가능한 모든 변수를 JSON Schema 규격으로 정의합니다.
지원 타입: \`string\` (기본 입력, \`format:"textarea"\` 멀티라인, \`format:"uri"\` 이미지URL), \`number\` (기본 ±스테퍼, \`"ui:widget":"slider"\` 슬라이더), \`boolean\` (토글), \`color\` (컬러피커 — 배경색, 텍스트색, 그라데이션 색상 등 **모든 색상 값은 반드시 이 타입 사용**), \`string + enum\` (드롭다운), \`array\` (동적 목록).
...
\`\`\`json
{
  "html": "<div id=\\"overlay\\">\\n  <span data-cg-bind=\\"teamName\\">WebCG-K</span>\\n</div>",
...
function extractJsDataKeys(js: string): Set<string> {
	const keys = new Set<string>();
	// 1. data.xxx 형태 추출
	for (const match of js.matchAll(/\bdata\.([A-Za-z_$][\w$]*)/g)) {
		keys.add(match[1]);
	}
	// 2. data['xxx'] 또는 data["xxx"] 형태 추출
	for (const match of js.matchAll(/\bdata\[['"]([^'"]+)['"]\]/g)) {
		keys.add(match[1]);
	}
	// 3. const { xxx, yyy: zzz } = data; 와 같은 구조 분해 할당(Destructuring) 추출
	const destructurePattern = /(?:const|let|var)\s*\{\s*([^}]+)\s*\}\s*=\s*data\b/g;
	for (const match of js.matchAll(destructurePattern)) {
		const properties = match[1];
		for (const prop of properties.split(",")) {
			const trimmed = prop.trim();
			if (!trimmed) continue;
			// 별칭(alias)이 있는 경우 (예: timerDuration: duration) 콜론 기준 왼쪽을 원본 키로 채택
...
export function validateOverlayBindings(result: OverlayCodeResult): OverlayBindingValidation {
	...
	for (const key of schemaKeys) {
		// Heuristic Fallback: JS 구조 분해 할당이나 AST 기반 정적 추출에서 놓칠 수 있는 
		// JS 내 특수 바인딩(예: dynamic key)이나 CSS variables 테마 적용을 위한 2차 휴리스틱 구제
		const isUsedInJs = jsKeys.has(key) || (result.js && new RegExp(`\\b${key}\\b`).test(result.js));
		const isUsedInCss = result.css && new RegExp(`\\b${key}\\b`).test(result.css);

		if (!bindKeys.has(key) && !isUsedInJs && !isUsedInCss) {
			missingBindings.push(key);
			warnings.push(`스키마 키 "${key}"가 HTML(data-cg-*), JS 또는 CSS에서 사용되지 않습니다.`);
		}
		if (!Object.prototype.hasOwnProperty.call(defaults, key)) {
			missingDefaults.push(key);
			warnings.push(`스키마 키 "${key}"의 replicant_defaults 기본값이 없습니다.`);
		}
	}
	...
	for (const key of bindKeys) {
		if (!schemaKeySet.has(key)) {
			...
			warnings.push(`HTML 바인딩 "${key}"에 대응하는 스키마 필드가 없습니다.`);
		}
	}

	// [Senior Mentor Decision (ADR)]
	// 단순 미사용 스키마 키(missingBindings)는 검증 실패 경고(ok = false)로 다스리지 않고,
	// 오직 치명적인 스키마 누락(orphanBindings)만 실패로 판정하여 빌드 ok: true 상태를 유지합니다.
	// 이는 AI 모델이 자유롭게 창의적 스타일링과 미학적 애니메이션 상태 관리를 하도록 자율성을 부여합니다.
	return {
		ok: orphanBindings.length === 0,
		warnings,
		missingBindings,
		orphanBindings,
		missingDefaults,
