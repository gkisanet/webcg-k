...
export async function runPreflight(
	...
	const bundleAvailable = Boolean(bundleId);

	// 1단계: 번들 슬롯 조회. 런다운 화면은 번들 매핑 없이 콘텐츠만 검증할 수 있다.
	let slots: (BundleSlot & {
	...
	if (bundleAvailable) {
		...
		slots = (slotsRaw || []) as unknown as (BundleSlot & {
			graphics?: {
				name: string;
	...
	const itemResults: PreflightItemResult[] = items.map((item) => {
		...
		if (!bundleAvailable) {
			return {
				...
				cgResults: [],
				contentIssues: [],
				status: requireBundle ? ("error" as const) : ("ok" as const),
			};
		}
