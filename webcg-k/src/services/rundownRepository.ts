...
export interface RundownSectionImportPayload extends RundownSectionImportCandidate {
  items: RundownItem[];
}
...
export async function fetchRundownSectionImportCandidates(params: {
  ...
  let query = supabase
    .from("rundowns")
    .select("id, title, description, is_public, workspace_id, sections_data, updated_at")
  ...
  const candidates = rows.flatMap((row) => {
    const sourceSections = Array.isArray(row.sections_data) ? row.sections_data : [];
  ...
  const sourceRundownIds = [...new Set(candidates.map((candidate) => candidate.source_rundown_id))];
  ...
  for (const item of (itemRows || []) as Array<{ rundown_id: string; section_id: string | null }>) {
  ...
  return candidates.map((candidate) => ({
    ...candidate,
    item_count: countBySection.get(`${candidate.source_rundown_id}:${candidate.section.id}`) || 0,
  }));
}
...
export async function fetchRundownSectionImportPayload(
  ...
  const sourceSections = Array.isArray(meta.sections_data) ? meta.sections_data : [];
  const section = sourceSections.find((candidate) => candidate.id === sourceSectionId);
  if (!section) {
    throw new Error("가져올 섹션을 찾을 수 없습니다.");
  }

  const sourceItems = await fetchRundownItems(sourceRundownId);
  const items = sourceItems.filter((item) => item.section_id === sourceSectionId);
