...
export interface SrcdocOptions {
	...
	/** manifest 기반 객체 타임라인. runtime 주입 후 사용자 JS 이전에 등록된다. */
...
export function buildPluginSrcdoc({
  ...
  color-scheme: normal;
	...
	const extraScripts = extraBodyScripts
		...
		.join("\n");
	const motionTimeline = graphicMotionManifestToRuntimeTimeline(motion);
	const motionTimelineScript =
		motionTimeline.length > 0
			? `<script>try { window.webcgk && window.webcgk.motion && window.webcgk.motion.setTimeline(${stringifyForInlineScript(motionTimeline)}); } catch(e) { console.error("[webcgk-motion]", e); }</script>`
			: "";
