...
export function renderSvgCompositionMaskShape<E extends SvgCompositionElement>(
	...
	return <path key={`mask-path-${element.id}`} d={pathD} fill={fill} transform={transform} />;
}
...
function renderDefaultBooleanFill<E extends SvgCompositionElement>(
	...
	const fill =
		base.fill?.type === "linear" || base.fill?.type === "radial"
			? (idPrefix ? `url(#gradient-${idPrefix}-${base.id})` : `url(#gradient-${base.id})`)
			: base.fill?.type === "none"
				? "transparent"
				: base.fill?.color || "#333";
	return (
...
export function renderSvgBooleanGroup<E extends SvgCompositionElement>({
	...
	const renderBooleanMaskShape = (operand: E, fill: string) => {
		...
		const transform = operand.rotation ? `rotate(${operand.rotation} ${cx} ${cy})` : undefined;
		...
		if (!d) return null;
		return <path key={`boolean-path-${operand.id}`} d={d} fill={fill} transform={transform} />;
	};
	...
	const maskContent =
		operation === "subtract" ? (
			<>
				...
				{operands.slice(1).map((operand) => renderBooleanMaskShape(operand, "black"))}
			</>
		) : (
	...
	return (
		<g key={element.id} {...groupProps}>
			...
			{Boolean(operands.length) && <g mask={`url(#${maskId})`}>{booleanFill}</g>}
			{trailingNode}
		</g>
	);
}
