...
describe("timelineStore playout actions", () => {
	...
	it("marks unaired blocks between broadcasts as skipped", () => {
		...
		expect(timelineStore.state.airedBlockIds).toEqual(
			new Set(["intro", "outro"]),
		);
		expect(timelineStore.state.completedBlockIds).toContain("intro");
		expect(timelineStore.state.skippedBlockIds).toEqual(
			new Set(["lower-third"]),
		);
	});
	...
	it("adjusts playhead and last broadcast positions after ripple deleting a gap", () => {
		...
		expect(
			timelineStore.state.blocks.find((b) => b.id === "after")?.startPosition,
		).toBe(150);
	...
	it("keeps manual whiteboard PGM active when broadcasting from an empty timeline position", () => {
		resetTimeline({
			blocks: [block("caption", 0, 1)],
			playheadPosition: 300,
			pgmBlockIds: new Map([
				[1, "caption"],
				[99, "wb-pgm-board-a"],
			]),
		...
		expect(timelineStore.state.pgmBlockIds).toEqual(
			new Map([[99, "wb-pgm-board-a"]]),
		);
		expect(timelineStore.state.completedBlockIds).toContain("caption");
		expect(timelineStore.state.completedBlockIds).not.toContain(
			"wb-pgm-board-a",
		);
	});
});
