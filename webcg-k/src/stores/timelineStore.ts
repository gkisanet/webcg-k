...
export function broadcastToPGM(): void {
	timelineStore.setState((state) => {
		...
		// 블록이 하나도 없으면 타임라인 그래픽만 소거하고, 수동 판서 트랙은 유지한다.
		if (activeBlocks.length === 0) {
			...
			return {
				...state,
				pgmBlockIds: new Map(
					[...newPgmBlockIds].filter(([trackId]) => trackId === 99),
				),
...
export function moveBlockToUpperTrack(): boolean {
	...
	const upperTrack = state.tracks.find(
		(t) => t.id > selectedBlock.trackId && !t.isLogoTrack,
	);
...
export function deleteSelectedBlock(): boolean {
	...
	timelineStore.setState((state) => ({
		...
		pgmBlockIds: newPgmBlockIds,
		previewBlockId:
			state.previewBlockId === state.selectedBlockId
				? null
				: state.previewBlockId,
	}));
 ...
 */
export function selectGapAtPosition(
	trackId: number,
	...
	for (const block of trackBlocks) {
		if (
			clickPosition >= block.startPosition &&
			clickPosition < block.startPosition + block.width
		) {
			// 블록 위를 클릭 - 갭 선택 취소
			return false;
		}
	...
	for (let i = 0; i < trackBlocks.length; i++) {
		...
		if (i < trackBlocks.length - 1) {
			...
			if (
				clickPosition >= currentEnd &&
				clickPosition < nextBlock.startPosition
			) {
	...
	if (
		...
		clickPosition < trackBlocks[0].startPosition &&
		clickPosition >= 0
	) {
		...
		console.log("Gap selected (before first block):", {
			start: 0,
			end: snappedEnd,
		});
		return true;
...
export function rippleDeleteGap(): boolean {
	...
	timelineStore.setState((state) => {
		...
		const nextPgmBlockIds = new Map(
			[...state.pgmBlockIds].filter(([, blockId]) =>
 ...
 */
export function expandLogoToSegment(
	logoBlockId: string,
	segmentId: string,
): void {
	const state = timelineStore.state;
	...
	const segMaxEnd = Math.max(
		...segBlocks.map((b) => b.startPosition + b.width),
	);
...
export function reorderBlocksBySegments(newSegments: Segment[]): void {
	...
	timelineStore.setState((state) => {
		...
		for (const seg of sorted) {
			...
			const lastRelativeEnd =
				lastBlock.startPosition - originalStart + lastBlock.width;
...
function areSnapshotsEqual(
	...
	if (JSON.stringify(s1.selectedGap) !== JSON.stringify(s2.selectedGap))
		return false;
...
export function pushToHistory(): void {
	...
	console.log(
		`[History] State pushed. Undo: ${pastStates.length}, Redo: ${futureStates.length}`,
	);
}
...
export function undo(): boolean {
	...
	timelineStore.setState((state) => ({
		...
		selectedGap: previousState.selectedGap
			? { ...previousState.selectedGap }
			: null,
		historyVersion: state.historyVersion + 1, // 리렌더링 트리거
	}));

	console.log(
		`[History] Undo executed. Undo: ${pastStates.length}, Redo: ${futureStates.length}`,
	);
	return true;
}
...
export function redo(): boolean {
	...
	console.log(
		`[History] Redo executed. Undo: ${pastStates.length}, Redo: ${futureStates.length}`,
	);
	return true;
}
