...
function GraphicsPage() {
	...
	const handleAiSaveVariation = async (
		...
		const data = await saveAiVariation(variation, { gridId: meta.gridId, zoneBounds: meta.zoneBounds }, user.id);
	...
	const handleForkGraphic = async (item: Graphic & { _type: ViewMode }) => {
		...
		try {
			...
			const data = await cloneGraphic({ name: item.name, description: item.description, template_data: item.template_data }, user.id);
	...
	const { data: graphics = [], isLoading: graphicsLoading } = useQuery({
		...
		queryFn: async () => {
			...
			return (data as Graphic[]).map((g) => ({ ...g, _type: "gallery" as const }));
		},
	})
	...
	const isLoading = viewMode === "grid-templates"
		? templatesLoading
		: viewMode === "bundles"
			? bundlesLoading
			: graphicsLoading;
			...
			columnHelper.display({
				...
				cell: ({ row }) => {
					if (usedIdSet.has(row.original.id)) {
						return (
							<span className="owner-badge mine" style={{ background: "rgba(99,102,241,0.15)", color: "#a5b4fc" }}>
								사용 중
							</span>
						);
					}
	...
	const handleDelete = async (item: ListItem) => {
		...
		try {
			...
			queryClient.invalidateQueries({ queryKey: [item._type === "gallery" ? "graphics" : "gridTemplates"] });
		} catch (error) {
		...
		<>
            ...
            <div className="dash-page-header">
				<div>
					...
					<div className="dash-page-subtitle">
						방송에 사용될 벡터 그래픽 디자인 에셋을 관리하고 AI로 자동 생성합니다.
					</div>
				</div>
			...
			<div className="graphics-split-view">
				...
				<div className="graphics-table-panel">
					...
					{/* 빈 상태 */}
					{!isLoading && (viewMode === "bundles" ? bundleListItems.length === 0 : currentData.length === 0) && (
					...
					{/* 번들 테이블 */}
					{!isLoading && viewMode === "bundles" && bundleListItems.length > 0 && (
						<>
								...
								<table className="graphics-table">
									...
									<tbody>
										{bundleTable.getRowModel().rows.map((row) => {
											const isSelected = selectedBundle?.id === row.original.id;
											return (
												<tr
													key={row.id}
						...
						<>
							...
							<div className="preview-thumbnail">
								{selectedItem._type === "grid-templates" ? (
									...
									<img src={selectedItem.thumbnail_path} alt={selectedItem.name} />
								) : (
							...
							<div className="preview-details">
								...
								{selectedItem._type === "grid-templates" && (selectedItem as GridTemplateRow).owner_id === user?.id ? (
									...
									<div className="template-settings">
										<div className="setting-group">
											...
											<textarea
												...
												onBlur={(e) => {
													updateGraphicMutation.mutate({ id: selectedItem.id, description: e.target.value });
												}}
					...
					>
						...
						<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
							<div>
								...
								<Input value={bundleFormName} onChange={(e) => setBundleFormName(e.target.value)} placeholder="예: KBS 뉴스9 CG 세트" />
							</div>
							<div>
								<label style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 4, display: "block" }}>
									대상 프로그램 (NRCS 매칭용)
								</label>
								<Input value={bundleFormProgram} onChange={(e) => setBundleFormProgram(e.target.value)} placeholder="예: KBS 뉴스 9" />
							</div>
							<div>
								<label style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 4, display: "block" }}>
									설명 (선택)
								</label>
								<Input value={bundleFormDesc} onChange={(e) => setBundleFormDesc(e.target.value)} placeholder="번들에 대한 설명" />
							</div>
						</div>
						<div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
							<Button variant="ghost" onClick={() => setShowBundleModal(false)}>취소</Button>
							<Button onClick={handleCreateBundle} disabled={!bundleFormName.trim()}>생성</Button>
						</div>
					</div>
				</div>
			)}
