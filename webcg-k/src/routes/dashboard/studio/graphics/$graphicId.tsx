...
export interface GraphicElement {
	...
	maskMode?: "alpha" | "clip";
	booleanOperation?: import("@/lib/element/vectorComposition").BooleanOperation;
	// 🆕 HTML 플러그인 (오버레이 임베딩)
	...
	shadowBlur?: number;
	shadowAngle?: number; // 🆕 각도 (0-360)
	shadowDistance?: number; // 🆕 거리 (px)
	...
	innerShadowAngle?: number; // 🆕 각도 (0-360)
	innerShadowDistance?: number; // 🆕 거리 (px)
	...
	const saveMutation = useMutation({
		mutationFn: async () => {
			...
			const { error: rundownError } = await supabase
				.from("rundown_items")
				.update({
					source_name: graphicName,
					data: nextTemplateData,
					thumbnail: null,
					updated_at: new Date().toISOString(),
				})
				.eq("source_type", "graphic")
				.eq("source_id", graphicId);
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({ queryKey: ["rundownItems"] });
				...
				<div className="header-center">
					...
					<div className="title-group">
						...
						<NamingSearchBox
							...
							placeholder="좌상단-헤드라인-두글자..."
							value={graphicName}
							onChange={(value) => {
								setGraphicName(value);
								setHasChanges(true);
							}}
							clearLabel="그래픽 이름 지우기"
							showLeadingIcon={false}
							suggestionTitle="그래픽 이름 만들기"
							suggestionHint="추천 토큰을 선택해 이름을 표준 형식으로 맞춥니다."
						/>
					</div>
