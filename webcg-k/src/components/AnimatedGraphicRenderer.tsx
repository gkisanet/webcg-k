...
import {
	...
	memo,
	useEffect,
	useId, // 🆕 useId 추가로 그라데이션 및 효과 ID 격리화 대응
	useLayoutEffect,
	useMemo,
	useRef,
	useState,
...
import {
	resolveCornerRadius,
	toCssBorderRadius,
} from "@/lib/element/cornerRadius";
import {
	getSvgCompositionChildren,
	renderSvgBooleanGroup,
	renderSvgCompositionGroup, // 🆕 공통 SVG 컴포지션 렌더러 임포트 추가
...
function composeKeyframesWithBaseTransform(
	keyframes: AnimKeyframes,
	baseTransform?: string,
): AnimKeyframes {
	const normalizedBase = baseTransform?.trim();
	if (!normalizedBase || normalizedBase === "none") return keyframes;
	...
	return keyframes.map((frame) => {
		...
		return {
			...frame,
			transform: `${transform} ${normalizedBase}`,
		};
	});
}
...
function buildAnimOptions(
	...
	if ((phase === "enter" || phase === "idle") && anim.loop) {
		const loopDelay = anim.enter
			? (anim.enter.duration ?? DEFAULT_ENTER_DURATION) +
				(anim.enter.delay ?? 0)
			: 0;
		result.loop = {
			...
			fill: "none",
			iterations:
				anim.loop.iterationCount === "infinite"
					? Infinity
					: (anim.loop.iterationCount ?? Infinity),
		};
	}
...
function applyAnimation(
	...
	const start = (
		...
		const a = el.animate(
			composeKeyframesWithBaseTransform(keyframes, el.style.transform),
			{
...
function generateFallbackKeyframeStyles(): string {
	...
	if (twKf) {
		...
		rules.push(
			`@keyframes enter-typewriter { from { ${Object.entries(from)
				.map(
					([k, v]) =>
						`${k.replace(/[A-Z]/g, (m) => `-${m.toLowerCase()}`)}:${v}`,
				)
				.join(";")}; } to { ${Object.entries(to)
				.map(
					([k, v]) =>
						`${k.replace(/[A-Z]/g, (m) => `-${m.toLowerCase()}`)}:${v}`,
				)
	...
	if (shKf) {
		...
		rules.push(
			`@keyframes loop-shimmer { from { ${Object.entries(from)
				.map(
					([k, v]) =>
						`${k.replace(/[A-Z]/g, (m) => `-${m.toLowerCase()}`)}:${v}`,
				)
...
function buildFallbackCSS(
	...
	if (
		...
		const loopDelay = anim.enter
			...
			: 0;
		parts.push(
			`loop-shimmer ${duration}ms ease-in-out ${loopDelay}ms infinite`,
		);
	}
...
function fillToBackground(fill?: GraphicElement["fill"]): string {
	...
	// 🆕 선형 그라데이션 (CSS linear-gradient 변환)
	if (fill.type === "linear") {
	...
	// 🆕 원형 그라데이션 (CSS radial-gradient 변환)
	if (fill.type === "radial") {
		const stops = fill.gradientStops || [
			{ offset: 0, color: "#3b82f6" },
			{ offset: 100, color: "#8b5cf6" },
...
export const AnimatedGraphicRenderer = memo(function AnimatedGraphicRenderer({
	...
	// 🆕 고유 ID 생성 (동일 화면 다중 SVG 합성 렌더러 그라데이션/효과 ID 충돌 완전 격리)
	...
	// Why: HTML/DOM 기반 가속 렌더링 시에도, boolean_group 이나 group 등 내부 SVG 합성 연산이
	//      그라데이션 defs를 무결하게 참조하여 드로우되도록 글로벌 defs 공간을 확보합니다.
					...
					if (el.fill?.type === "linear") {
						return (
							...
							>
								{stops.map((stop) => (
									<stop
										key={`${el.id}-linear-${stop.offset}-${stop.color}-${stop.opacity ?? 1}`}
										offset={`${stop.offset}%`}
						...
						return (
							...
							>
								{stops.map((stop) => (
									<stop
										key={`${el.id}-radial-${stop.offset}-${stop.color}-${stop.opacity ?? 1}`}
										offset={`${stop.offset}%`}
										stopColor={stop.color}
										stopOpacity={stop.opacity ?? 1}
									/>
	...
	useLayoutEffect(() => {
		...
		const updateScale = (parentW: number, parentH: number) => {
	...
	useEffect(() => {
		...
		for (const el of elements) {
			...
			const domEl = containerRef.current.querySelector(
	...
	const getChildren = (parentId: string): GraphicElement[] =>
		getSvgCompositionChildren(elements, parentId);
		...
		if (element.dataBinding?.field && data) {
			...
			if (value !== undefined) {
				if (element.dataBinding.format) {
					// 포맷 패턴 적용 (예: "{value}°C")
					content = element.dataBinding.format.replace(
						"{value}",
						String(value),
					);
				} else {
	...
	const renderElement = (element: GraphicElement): React.ReactNode => {
		...
		const combinedFilter =
			[element.filter, editorShadowCSS, editorGlowCSS]
				.filter(Boolean)
				.join(" ") || undefined;
		...
		const baseRadius = resolveCornerRadius(element, {
			width: element.width,
			height: element.height,
		});
		const baseStyle: CSSProperties = {
			position: element.position || "absolute",
			borderRadius: toCssBorderRadius(baseRadius),
			overflow: baseRadius.hasAny ? "hidden" : undefined,
			...
			// 🆕 고급 시각 효과 (Phase 2 확장)
			boxShadow:
				[element.boxShadow, editorInnerShadowCSS].filter(Boolean).join(", ") ||
				undefined,
			...
			mixBlendMode: ((element as any).blendMode &&
			(element as any).blendMode !== "normal"
		...
		switch (element.type) {
			case "rect": {
				...
				const renderShapeWidth = hasSlots
					...
					: element.width;
				const rectRadius = resolveCornerRadius(element, {
				...
				return (
					<div
						...
						style={{
							...baseStyle,
							width: `${renderShapeWidth}px`,
							background: bg,
							opacity: element.opacity * fillOpacity,
							borderRadius: toCssBorderRadius(rectRadius),
							...
							...(element.stroke?.enabled && strokeOpacity < 1
								? {
										borderColor: `${element.stroke.color || "#000"}${Math.round(
											strokeOpacity * 255,
										)
											.toString(16)
											.padStart(2, "0")}`,
								...
								: {}),
							// 슬롯이 있거나 코너 라운드가 적용되었으면 자식 넘침 방지(hidden) 처리
							overflow: hasSlots || rectRadius.hasAny ? "hidden" : undefined,
						}}
					>
						...
						{hasSlots &&
							bindingSlots.map((slot) => {
									...
									return (
										<div
											...
											style={{
												...
												left: `${slot.frameX}px`,
												top: `${slot.frameY}px`,
												width: `${layout.renderFrameWidth}px`,
												height: `${layout.renderFrameHeight}px`,
												color: slot.color,
								...
								// shrink / expandRightThenShrink 모드: 공통 정책의 scaleX 압축
								const scaleX = layout.textScaleX;
								return (
									<div
										key={slot.id}
										data-slot-id={slot.id}
			...
			case "image": {
				...
				return (
					<div key={element.id} data-element-id={element.id} style={baseStyle}>
						{imgSrc && (
							<img
								src={imgSrc}
								alt={element.name}
			...
			case "boolean_group": {
				const content = renderSvgBooleanGroup({
					element,
					elements,
					idPrefix: uniqueId, // 🆕 하드코딩 "animated" 대신 고유 ID 부여
				});
				...
				return (
					<svg
						...
						data-element-id={element.id}
						viewBox={`${element.x} ${element.y} ${element.width} ${element.height}`}
						style={{
							...baseStyle,
							overflow: "visible",
						}}
	...
	return (
		...
		>
			{/* 🆕 보이지 않는 SVG를 통한 그라데이션 defs 전역 주입 */}
			<svg
				style={{
					position: "absolute",
					width: 0,
