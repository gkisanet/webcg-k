    ...
    <div className="flex flex-col h-full gap-3">
      ...
      <div className="flex-1 min-h-0 flex gap-3">
        <div className="flex-1 min-w-0 flex flex-col gap-2">
          <div ref={containerRef} className="flex-1 min-h-0 relative rounded-xl overflow-hidden border border-[var(--border-primary)] bg-black">
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            {activeState?.status === "done" && <>
              ...
              <div className="flex items-center gap-1.5 ml-auto"><input type="text" value={modifyPrompt} onChange={(e) => setModifyPrompt(e.target.value)} placeholder="Modify request..." className="w-48 px-2 py-1 rounded bg-[var(--app-bg)] border border-[var(--border-primary)] text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)]" onKeyDown={(e) => { if (e.key === "Enter") handleModify(activeSceneIdx); }} /><Button size="sm" variant="ghost" onClick={() => handleModify(activeSceneIdx)} disabled={!modifyPrompt.trim()}>Apply</Button></div>
            </>}
          </div>
        </div>

        <div className="w-96 shrink-0 flex flex-col gap-3 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-xl p-3 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {rightTab === "info" ? <div className="space-y-4 pr-1 text-left">
              <div className="space-y-2"><div className="flex justify-between items-center text-[11px] text-[var(--text-muted)] border-b border-[var(--border-primary)] pb-1.5"><span>Scene Order</span><span className="font-semibold text-[var(--text-primary)]">#{activeScene.order}</span></div>{activeScene.graphic_type && <div className="flex justify-between items-center text-[11px] text-[var(--text-muted)] border-b border-[var(--border-primary)] pb-1.5"><span>Type</span><span className="px-1.5 py-0.5 rounded-md text-[10px] bg-blue-500/10 text-blue-400 font-mono">{activeScene.graphic_type}</span></div>}</div>

              <div className="space-y-2 rounded-lg border border-[var(--border-primary)] bg-[var(--app-bg)] p-2">
                <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-1.5 text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider"><Grid3X3 size={12} /> Session Zones</div><span className="text-[9px] text-[var(--text-muted)]">{zoneProfile.name}</span></div>
                <select value="" onChange={(e) => handleGridTemplateSelect(e.target.value)} disabled={gridLoading} className="w-full h-7 rounded bg-[var(--bg-secondary)] border border-[var(--border-primary)] text-[10px] text-[var(--text-primary)] px-2"><option value="">{gridLoading ? "Loading grids..." : "기존 Grid Template 불러오기"}</option>{gridTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select>
                <div className="space-y-1.5">{AI_CUESHEET_ZONE_ORDER.map((zone) => { const def = getZoneDefinition(zoneProfile, zone); return <div key={zone} className="rounded border border-[var(--border-primary)]/60 bg-[var(--bg-secondary)] p-1.5"><div className="flex items-center justify-between mb-1"><span className="text-[9px] font-mono text-blue-300">{zone}</span><span className="text-[8px] text-[var(--text-muted)] truncate max-w-[120px]">{def.sourceZoneName ?? "session"}</span></div><div className="grid grid-cols-4 gap-1">{(["x", "y", "width", "height"] as const).map((field) => <label key={field} className="text-[8px] text-[var(--text-muted)]">{field}<input type="number" value={def.bounds[field]} onChange={(e) => updateZoneBounds(zone, field, Number(e.target.value))} className="mt-0.5 w-full rounded bg-black/20 border border-[var(--border-primary)] px-1 py-0.5 text-[9px] text-[var(--text-primary)]" /></label>)}</div></div>; })}</div>
              </div>

              <div className="space-y-1"><div className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Trigger Timing</div><div className="p-2 rounded-lg bg-[var(--app-bg)] border border-[var(--border-primary)] text-xs text-[var(--text-primary)] leading-relaxed">{activeScene.trigger}</div></div>
              <div className="space-y-1"><div className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Design Intent</div><div className="p-2 rounded-lg bg-[var(--app-bg)] border border-[var(--border-primary)] text-xs text-[var(--text-secondary)] leading-relaxed">{activeScene.graphic_intent}</div></div>
              <div className="space-y-2"><div className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Text Slots ({activeScene.text_slots.length})</div><div className="space-y-2">{activeScene.text_slots.map((slot, slotIdx) => { const displayVal = slot.display_value ?? slot.value; const zoneDef = getZoneDefinition(zoneProfile, slot.zone_hint); return <div key={slot.id ?? slotIdx} className="p-2 rounded-lg bg-[var(--app-bg)] border border-[var(--border-primary)] space-y-1.5 text-left"><div className="flex justify-between items-center flex-wrap gap-1"><span className="px-1.5 py-0.5 rounded text-[9px] bg-gray-500/10 text-gray-400 font-mono">{slot.semantic_role}</span><span className="text-[8px] text-[var(--text-muted)]">{formatZoneDefinitionForPrompt(zoneDef)}</span></div><div className="text-[11px] text-[var(--text-primary)] font-medium leading-relaxed bg-[var(--bg-secondary)] p-1.5 rounded border border-[var(--border-primary)] break-all">{displayVal}</div><div className="grid grid-cols-[1fr_1fr_64px] gap-1.5"><select value={slot.zone_hint} onChange={(e) => onUpdateSlot(activeSceneIdx, slotIdx, { zone_hint: e.target.value as ZoneHint })} className="h-6 rounded bg-[var(--bg-secondary)] border border-[var(--border-primary)] text-[10px] text-[var(--text-primary)] px-1">{ZONES.map((z) => <option key={z} value={z}>{z}</option>)}</select><select value={slot.style_hint} onChange={(e) => onUpdateSlot(activeSceneIdx, slotIdx, { style_hint: e.target.value as StyleHint })} className="h-6 rounded bg-[var(--bg-secondary)] border border-[var(--border-primary)] text-[10px] text-[var(--text-primary)] px-1">{STYLES.map((s) => <option key={s} value={s}>{s}</option>)}</select><select value={String(slot.importance)} onChange={(e) => onUpdateSlot(activeSceneIdx, slotIdx, { importance: Number(e.target.value) })} className="h-6 rounded bg-[var(--bg-secondary)] border border-[var(--border-primary)] text-[10px] text-[var(--text-primary)] px-1">{[1, 2, 3, 4, 5].map((v) => <option key={v} value={v}>imp {v}</option>)}</select></div></div>; })}</div></div>
            </div> : <div className="space-y-3">{Object.keys(extractedThemes).length === 0 ? <p className="text-[11px] text-[var(--text-muted)] leading-relaxed text-left">Generate a graphic first, then click Extract Theme to save its color palette.</p> : <div className="space-y-2">{Object.entries(extractedThemes).map(([id, theme]) => <div key={id} className="p-2.5 rounded-lg bg-[var(--app-bg)] border border-[var(--border-primary)] text-left"><div className="flex gap-1 mb-1.5">{[theme.colors.primary, theme.colors.accent, theme.colors.background, theme.colors.text.main].map((c, ci) => <div key={ci} className="w-5 h-5 rounded ring-1 ring-white/10" style={{ backgroundColor: c }} />)}</div><div className="text-[10px] text-[var(--text-muted)] mb-1.5">{theme.typography.fontFamily} · r={theme.layout.borderRadius}</div><Button size="sm" variant="ghost" className="w-full text-[10px] h-6" onClick={() => handleGenerate(activeSceneIdx, id)} disabled={activeState?.status === "generating"}>Apply to Scene</Button></div>)}</div>}</div>}
          </div>
        </div>
      </div>
      ...
      {codeEditorOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="w-[min(1180px,96vw)] h-[min(760px,90vh)] rounded-xl border border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-2xl flex flex-col overflow-hidden">
            <div className="h-12 shrink-0 flex items-center justify-between px-4 border-b border-[var(--border-primary)]">
            ...
            <div className="flex-1 min-h-0 grid grid-cols-[1fr_420px]">
              ...
              <iframe
                ...
                className="w-full h-full border-l border-[var(--border-primary)] bg-black"
