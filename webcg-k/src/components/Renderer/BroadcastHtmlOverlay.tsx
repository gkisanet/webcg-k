...
export type BroadcastHtmlOverlayPhase = "enter" | "idle" | "exit";

interface BroadcastHtmlOverlayProps {
	payload: BroadcastOverlayPayload;
	title: string;
	phase?: BroadcastHtmlOverlayPhase;
	width?: number;
	height?: number;
...
export function BroadcastHtmlOverlay({
	...
	const srcdoc = useMemo(
		...
		[payload.html, payload.css, payload.js, payload.motion, width, height],
	);

	const postToIframe = useCallback(
		(type: "INIT" | "REPLICANT_UPDATE" | "SHOW" | "HIDE") => {
			const target = iframeRef.current?.contentWindow;
			if (!target) return;
			const message =
				type === "INIT" || type === "REPLICANT_UPDATE"
					? { type, payload: JSON.parse(dataJson) }
					: { type };
		...
		>
			<iframe
				...
				onLoad={() => {
					postToIframe("INIT");
					setTimeout(() => {
						postToIframe(phase === "exit" ? "HIDE" : "SHOW");
					}, 50);
				}}
			/>
		</div>
