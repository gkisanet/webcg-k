...
} from "@/lib/element/elementOperations";
import {
	...
	isBooleanOperand,
	isMaskSource,
} from "@/lib/element/vectorComposition";
...
export function GraphicsEditor({
	...
	const [activeTool, setActiveTool] = useState<
		"select" | "rect" | "ellipse" | "text" | "image" | "html_plugin"
	>("select");
	const centerRef = useRef<HTMLDivElement>(null);
	const [zoom, setZoom] = useState(1);
	const [fitZoom, setFitZoom] = useState(1);
	const [motionPreviewPhase, setMotionPreviewPhase] = useState<
		"enter" | "exit" | null
	>(null);
	const [motionPreviewKey, setMotionPreviewKey] = useState(0);
	const [layersPanelMode, setLayersPanelMode] = useState<"layers" | "motion">(
		"layers",
	);
		...
		try {
			...
			if (!error && images) {
				const imagesWithUrls = images.map((img: any) => ({
					id: img.id,
					name: img.name,
					url: supabase.storage.from("images").getPublicUrl(img.storage_path)
						.data.publicUrl,
				}));
	...
	const handleAddElementOrOpenPicker = useCallback(
		(type: GraphicElement["type"]) => {
			if (type === "image") {
	...
	const reorderMotionElements = useCallback(
		(phase: "enter" | "exit", orderedIds: string[], stepMs: number) => {
			const delayById = new Map(
				orderedIds.map((id, index) => [id, index * stepMs]),
			);

			onElementsChange(
				elements.map((el) => {
					const delay = delayById.get(el.id);
					if (delay === undefined || !el.animation?.[phase]) return el;
	...
	const removeMotionFromElements = useCallback(
		(ids: string[], phase?: "enter" | "exit") => {
			if (ids.length === 0) return;
			const targetIds = new Set(ids);

			onElementsChange(
				elements.map((el) => {
					if (!targetIds.has(el.id) || !el.animation) return el;

					const nextAnimation = { ...el.animation };
					if (!phase || phase === "enter") {
						delete nextAnimation.enter;
					}
					if (!phase || phase === "exit") {
						delete nextAnimation.exit;
					}
					...
					if (
						...
						return { ...el, animation: undefined };
					}

					return { ...el, animation: nextAnimation };
				}),
			);
		},
		[elements, onElementsChange],
	...
	const applyStaggeredMotion = useCallback(
		(
			phase: "enter" | "exit",
			orderedIds: string[],
			selectedIds: string[],
			...
			const currentDelayById = new Map(
				elements.map((el) => [el.id, el.animation?.[phase]?.delay ?? 0]),
			);
			const nextDelayById = new Map<string, number>();
			let previousDelay = 0;
			let hasPrevious = false;

			for (const id of orderedIds) {
				if (selectedIdSet.has(id)) {
					const nextDelay = hasPrevious ? previousDelay + stepMs : 0;
					nextDelayById.set(id, nextDelay);
					previousDelay = nextDelay;
					hasPrevious = true;
					continue;
				}

				previousDelay = currentDelayById.get(id) ?? previousDelay;
				hasPrevious = true;
			}

			onElementsChange(
				elements.map((el) => {
					const delay = nextDelayById.get(el.id);
					if (delay === undefined) return el;

					const animation: NonNullable<GraphicElement["animation"]> = {
						...(el.animation ?? {}),
						[phase]: {
							type:
								el.animation?.[phase]?.type ??
								(phase === "enter" ? "fadeIn" : "fadeOut"),
							duration:
								el.animation?.[phase]?.duration ??
								(phase === "enter" ? 500 : 400),
							delay,
							easing:
								el.animation?.[phase]?.easing ??
								(phase === "enter" ? "ease-out" : "ease-in"),
						},
					};
	...
	const applySimultaneousMotion = useCallback(
		(phase: "enter" | "exit", orderedIds: string[], selectedIds: string[]) => {
			if (selectedIds.length === 0) return;
			const selectedIdSet = new Set(selectedIds);
			const currentDelayById = new Map(
				elements.map((el) => [el.id, el.animation?.[phase]?.delay ?? 0]),
			...
			for (const id of orderedIds) {
				if (selectedIdSet.has(id)) {
					const nextDelay = hasPrevious ? previousDelay : 0;
					nextDelayById.set(id, nextDelay);
					previousDelay = nextDelay;
					hasPrevious = true;
					continue;
	...
	const applySequenceMotion = useCallback(
		(phase: "enter" | "exit", orderedIds: string[], selectedIds: string[]) => {
			if (selectedIds.length === 0) return;
			const selectedIdSet = new Set(selectedIds);
			const currentDelayById = new Map(
			...
			const currentDurationById = new Map(
				elements.map((el) => [
					el.id,
					el.animation?.[phase]?.duration ?? (phase === "enter" ? 500 : 400),
				]),
			);
			const nextDelayById = new Map<string, number>();
			let previousDelay = 0;
			...
			for (const id of orderedIds) {
				const duration =
					currentDurationById.get(id) ?? (phase === "enter" ? 500 : 400);
				if (selectedIdSet.has(id)) {
					const nextDelay = hasPrevious ? previousDelay + previousDuration : 0;
					nextDelayById.set(id, nextDelay);
					previousDelay = nextDelay;
	...
	const createClipFrame = useCallback(() => {
		const selectedElems = elements.filter((el) => selectedIds.includes(el.id));
		if (selectedElems.length === 0) return;

		if (selectedElems.length === 1 && selectedElems[0].type === "group") {
			onElementsChange(
				elements.map((el) =>
					el.id === selectedElems[0].id ? { ...el, clipContent: true } : el,
				),
			);
			setSelectedIds([selectedElems[0].id]);
			return;
		}

		const bounds = getElementsBounds(selectedElems);
		if (!bounds) return;

		const childIds = selectedElems.map((el) => el.id);
		const frameId = `frame-${Date.now()}`;
		const frameElement: GraphicElement = {
			id: frameId,
			type: "group",
			name: "프레임",
			x: bounds.x,
		...
		const updatedElements = elements.map((el) =>
			childIdSet.has(el.id) ? { ...el, parentId: frameId } : el,
		);

		onElementsChange([...updatedElements, frameElement]);
	...
	const createMask = useCallback(() => {
		...
		const maskSource = [...selectedElems]
			.filter(isMaskSource)
			.sort((a, b) => (b.zIndex ?? 0) - (a.zIndex ?? 0))[0];
		if (!maskSource) return;

		const targets = selectedElems.filter(
			(el) => el.id !== maskSource.id && canMaskTarget(el),
		);
		if (targets.length === 0) return;

		const childIds = [...targets.map((el) => el.id), maskSource.id];

		// ■ Why 마스크 그룹 Bounds는 마스크 소스의 크기여야 하는가? (Figma 스펙)
		//   마스킹 효과는 마스크 소스(Window) 밖을 보여줄 수 없는 시각 연산이므로,
		//   보이지 않는 타겟 요소들의 합집합 대신 마스크 소스 자체의 크기를 그룹 Bounds로 삼는 것이
		//   사용자 조작 일치도와 시각 피드백(라벨 배치) 정합성을 확보하는 정수입니다.
		const bounds = {
			x: maskSource.x,
			y: maskSource.y,
		...
		const maskGroupId = `mask-${Date.now()}`;
		const maskGroup: GraphicElement = {
			id: maskGroupId,
			type: "group",
			name: "마스크",
			x: bounds.x,
			...
			parentId: null,
			children: childIds,
			maskSourceId: maskSource.id,
			maskMode: "alpha",
		};

		const childIdSet = new Set(childIds);
		const updatedElements = elements.map((el) =>
			childIdSet.has(el.id) ? { ...el, parentId: maskGroupId } : el,
		);

		onElementsChange([...updatedElements, maskGroup]);
		setSelectedIds([maskGroupId]);
	}, [elements, selectedIds, onElementsChange]);

	const createBooleanGroup = useCallback(
		(operation: BooleanOperation) => {
			const operands = elements
				.filter((el) => selectedIds.includes(el.id) && isBooleanOperand(el))
				.sort((a, b) => (a.zIndex ?? 0) - (b.zIndex ?? 0));
			if (operands.length < 2) return;

			const bounds = getElementsBounds(operands);
			if (!bounds) return;
			...
			const childIds = operands.map((el) => el.id);
			const groupId = `boolean-${operation}-${Date.now()}`;
			const booleanGroup: GraphicElement = {
				id: groupId,
				type: "boolean_group",
				name: `불리언 ${operationLabels[operation]}`,
				x: bounds.x,
				y: bounds.y,
				width: bounds.width,
				height: bounds.height,
			...
			const childIdSet = new Set(childIds);
			const updatedElements = elements.map((el) =>
				childIdSet.has(el.id) ? { ...el, parentId: groupId } : el,
			);

			onElementsChange([...updatedElements, booleanGroup]);
			setSelectedIds([groupId]);
		},
		[elements, selectedIds, onElementsChange],
	);
	...
	const releaseMask = useCallback(() => {
		const nextElements = releaseCompositionMask(elements, selectedIds);
		if (nextElements === elements) return;
		onElementsChange(nextElements);
	}, [elements, selectedIds, onElementsChange]);

	const releaseBooleanGroup = useCallback(() => {
		const result = releaseBooleanGroups(elements, selectedIds);
		if (result.elements === elements) return;
		onElementsChange(result.elements);
		setSelectedIds(result.releasedChildIds);
	}, [elements, selectedIds, onElementsChange]);

	// 그룹 해제 (Ctrl+Shift+G) - 좌표는 그대로 유지
	const ungroupElements = useCallback(() => {
		const selectedElems = elements.filter(
			(el) =>
	...
	const duplicateElements = useCallback(() => {
		...
		// 캔버스 내 기존 엘리먼트 이름 목록 수집 (이름 중복 방지용)
		...
		const newElements = toDuplicate.map((el) => {
			...
			// 📐 방송 그래픽 특화형 스마트 서브 인덱스 명명 정책 적용 (예: 사각형2 -> 사각형2_1, 사각형2_2)
			let duplicatedName = el.name;

			// 1. 이미 서브 인덱스가 붙어있는지 확인 (예: "사각형2_1" 또는 "사각형_1")
			const subIndexMatch = el.name.match(/^(.*?)\s*_(\d+)$/);
			if (subIndexMatch) {
				const base = subIndexMatch[1].trim();
				let subNum = parseInt(subIndexMatch[2], 10) + 1;
				duplicatedName = `${base}_${subNum}`;

				// 이름 중복 회피 루프
				while (existingNames.has(duplicatedName)) {
					subNum++;
					duplicatedName = `${base}_${subNum}`;
				}
			} else {
				// 2. 서브 인덱스가 없는 경우 (예: "사각형2" 또는 "사각형")
				// 원본 이름을 base로 삼아 뒤에 "_1"을 붙이고 순차 증가시킴
				let subNum = 1;
				duplicatedName = `${el.name}_${subNum}`;
			...
			// 같은 선택 세트를 다중 복제할 때 새로 생성되는 이름들끼리 충돌하지 않도록 Set에 함께 기록
			existingNames.add(duplicatedName);

			return {
				...el,
				id: idMap.get(el.id)!,
				parentId: newParentId ?? null,
				name: duplicatedName, // 🆕 지능적으로 번호가 매겨진 새 이름
	...
	useEffect(() => {
		const unregDelete = registerAction({
			id: "deleteSelected",
			label: layersPanelMode === "motion" ? "선택 모션 삭제" : "선택 요소 삭제",
			shortcut: "Delete",
			context: "editor",
			predicate: () => selectedIds.length > 0,
			execute: () => {
				if (layersPanelMode === "motion") {
					removeMotionFromElements(selectedIds);
					return;
				}
		...
		deleteElements,
		layersPanelMode,
		removeMotionFromElements,
		duplicateElements,
		groupElements,
		ungroupElements,
	]);

	const canReleaseMask = selectedElements.some((element) =>
		Boolean(element.maskSourceId),
	);
	const canReleaseBooleanGroup = selectedElements.some(
		(element) => element.type === "boolean_group",
	);
		...
		<div className="graphics-editor">
			...
			<div className="graphics-editor-left">
				<ToolbarPanel
					...
					onBooleanOperation={createBooleanGroup}
					onCreateClipFrame={createClipFrame}
					onCreateMask={createMask}
					onReleaseMask={releaseMask}
					onReleaseBooleanGroup={releaseBooleanGroup}
					canReleaseMask={canReleaseMask}
					canReleaseBooleanGroup={canReleaseBooleanGroup}
				/>
				<LayersPanel
					elements={elements}
					...
					panelMode={layersPanelMode}
					onPanelModeChange={setLayersPanelMode}
					onMotionReorder={reorderMotionElements}
					onApplyStaggeredMotion={applyStaggeredMotion}
					onApplySimultaneousMotion={applySimultaneousMotion}
					onApplySequenceMotion={applySequenceMotion}
					onRemoveMotion={removeMotionFromElements}
			...
			{/* 중앙: 캔버스 */}
			<div
				className={`graphics-editor-center ${motionPreviewPhase ? "motion-preview-active" : ""}`}
				ref={centerRef}
			>
				{/* 스크롤 가능한 캔버스 영역 */}
				<div className="canvas-scroll-area">
					<Canvas
						...
						motionPreviewPhase={motionPreviewPhase}
						motionPreviewKey={motionPreviewKey}
					/>
				</div>
				<div className="motion-preview-controls">
					{motionPreviewPhase && (
						<span className="motion-preview-status">
							{motionPreviewPhase === "enter"
								? "IN 프리뷰 중"
								: "OUT 프리뷰 중"}
						</span>
					)}
					<button
						type="button"
						onClick={() => playMotionPreview("enter")}
						title="In 애니메이션 미리보기"
					>
						<Play size={13} />
						In 미리보기
					...
					<button
						type="button"
						onClick={() => playMotionPreview("exit")}
						title="Out 애니메이션 미리보기"
					>
						<Play size={13} />
						Out 미리보기
					</button>
				...
				<div className="zoom-controls">
					<button type="button" onClick={handleZoomOut} title="축소">
						−
					</button>
					<span className="zoom-level">{Math.round(zoom * 100)}%</span>
					<button type="button" onClick={handleZoomIn} title="확대">
						+
					</button>
					<button type="button" onClick={handleFitToView} title="화면에 맞춤">
						⊡
					</button>
					<button type="button" onClick={handleZoom100} title="100%">
						1:1
					</button>
				</div>
			</div>
								...
								<div className="image-picker-grid">
									{imageList.map((image) => (
										...
										>
											...
											<span className="image-picker-item-name">
												{image.name}
											</span>
										</div>
									))}
			...
			{showOverlayPicker && (
				<div
					className="image-picker-overlay"
					onClick={() => setShowOverlayPicker(false)}
				>
					<div
						className="image-picker-modal"
						onClick={(e) => e.stopPropagation()}
					>
						<div className="image-picker-header">
							<h3>
								<Layers size={18} style={{ marginRight: 6 }} /> 오버레이 선택
							</h3>
							<button
								type="button"
								onClick={() => setShowOverlayPicker(false)}
								...
								<div className="image-picker-grid">
									{overlayList.map((overlay) => (
										...
										>
											<Layers
												size={28}
												style={{ color: "#818cf8", marginBottom: 4 }}
											/>
											<span className="image-picker-item-name">
												{overlay.name}
											</span>
