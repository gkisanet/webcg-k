...
export function LayersPanel({
	...
	const renderLayerItem = (
		...
		const compositionBadges = [
			el.clipContent
				? { key: "clip", label: "Clip", title: "Clip Content 프레임" }
				: null,
			el.maskSourceId
				? { key: "mask", label: "Mask", title: "마스킹 그룹" }
				: null,
			el.type === "boolean_group"
				? {
						key: "boolean",
						label: booleanOperationLabels[el.booleanOperation ?? "union"],
						title: "Boolean Group",
					}
				: null,
			isMaskSource
				? { key: "mask-source", label: "Source", title: "마스크 소스" }
				: null,
			isBooleanOperand
				? { key: "operand", label: "Operand", title: "Boolean operand" }
				: null,
		].filter((badge): badge is { key: string; label: string; title: string } =>
			Boolean(badge),
		);
				...
				>
					...
					>
						{(() => {
							...
							const label =
								el.type === "text" && el.content ? el.content : el.name;
							// 15자 초과시 ...으로 줄임
	...
	return (
		<div className="layers-panel">
			...
			<div className={`shortcuts-guide ${isShortcutsExpanded ? "expanded" : "collapsed"}`}>
				<div
