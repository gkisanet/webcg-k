...
interface CanvasProps {
	...
	motionPreviewPhase?: "enter" | "exit" | null;
	motionPreviewKey?: number;
	onMotionPreviewExitComplete?: () => void;
}
...
export function Canvas({
	...
	canvasHeight,
	zoom = 1,
	motionPreviewPhase = null,
	motionPreviewKey = 0,
	onMotionPreviewExitComplete,
}: CanvasProps) {
	...
	const getAxisAlignedRectRendering = (
		element: GraphicElement,
		...radii: number[]
	): "crispEdges" | "geometricPrecision" => {
		const hasRotation = (element.rotation ?? 0) !== 0;
		const hasRadius = radii.some((radius) => radius > 0);
		return hasRotation || hasRadius ? "geometricPrecision" : "crispEdges";
	};
		...
		switch (element.type) {
			case "rect": {
				...
				const renderShapeWidth =
					bindingSlots.length > 0
						...
						: element.width;
				const radius = resolveCornerRadius(element, {
							...
							<rect
								...
								stroke={
									element.stroke?.enabled
										? element.stroke.color || "#1e40af"
										: "none"
								}
								strokeWidth={
									element.stroke?.enabled ? (element.stroke.width ?? 1) : 0
								}
								strokeDasharray={strokeDasharray}
								strokeOpacity={strokeOpacity}
								rx={radius.tl}
								shapeRendering={getAxisAlignedRectRendering(element, radius.tl)}
			...
			case "boolean_group": {
				return renderSvgBooleanGroup({
					...
					idPrefix: "editor",
					emptyNode: <g key={element.id} {...commonProps} />,
	...
	return (
		<div
			className={`canvas-container ${motionPreviewPhase ? "motion-previewing" : ""}`}
		...
		>
			...
			{motionPreviewPhase && (
				<div className="canvas-motion-preview-layer">
					<AnimatedGraphicRenderer
						key={`${motionPreviewPhase}-${motionPreviewKey}`}
						elements={elements}
						canvasWidth={canvasWidth}
						canvasHeight={canvasHeight}
						phase={motionPreviewPhase}
						onExitComplete={
							motionPreviewPhase === "exit"
								? onMotionPreviewExitComplete
								: undefined
						}
					/>
