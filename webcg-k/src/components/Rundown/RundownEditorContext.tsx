...
import type { RundownQualitySummary } from "@/lib/rundownQualityGate";
import type { CuesheetReusePolicy } from "@/services/cuesheetCheckService";
...
export interface RundownEditorState {
	...
	qualitySummary: RundownQualitySummary;
	qualityLoading: boolean;
	qualityValidationStale: boolean;
	qualityValidationApproved: boolean;
	qualityApproving: boolean;
...
export interface RundownEditorActions {
	setRundown: (
		val:
			| RundownMeta
			| null
			| ((prev: RundownMeta | null) => RundownMeta | null),
	) => void;
	setItems: (
		val: RundownItem[] | ((prev: RundownItem[]) => RundownItem[]),
	) => void;
	setSelectedItemId: (
	...
	setSectionImportScope: (
		val:
			| SectionImportScope
			| ((prev: SectionImportScope) => SectionImportScope),
	) => void;
	...
	refetchQualityGate: () => void;
	handleApproveQualityGate: () => Promise<void>;
	setReusePolicy: (policy: CuesheetReusePolicy) => void;
...
export function useRundownState(): RundownEditorState {
	...
	if (!context) {
		throw new Error(
			"useRundownState must be used within a RundownEditorProvider",
		);
	}
	return context;
}
...
export function useRundownActions(): RundownEditorActions {
	...
	if (!context) {
		throw new Error(
			"useRundownActions must be used within a RundownEditorProvider",
		);
	}
	return context;
}
