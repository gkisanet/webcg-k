...
function checkResolutionWarning(item: RundownItem): boolean {
	...
	const imageElements = (elements as Array<Record<string, unknown>>).filter(
		(el) => el.type === "image",
	);
...
function getGraphicThumbnailSource(item: RundownItem): {
	elements: GraphicElement[];
	canvasWidth: number;
	canvasHeight: number;
} | null {
	if (item.source_type !== "graphic" && item.source_type !== "template") {
		return null;
	}
	...
	return {
		...
		canvasWidth: data?.canvasWidth ?? canvas?.width ?? 1920,
		canvasHeight: data?.canvasHeight ?? canvas?.height ?? 1080,
	};
}
...
interface SortableRundownItemProps {
	...
	qualityStatus?: RundownQualityStatus;
...
const SortableRundownItem = memo(function SortableRundownItem({
	...
	const liveThumbnail = getGraphicThumbnailSource(item);
	...
	return (
		...
		>
			<button type="button" className="rundown-item-select" onClick={onSelect}>
				<span className="rundown-item-drag" {...attributes} {...listeners}>
					<GripVertical size={16} />
				</span>
				<span className="rundown-item-order">{index + 1}</span>
				{liveThumbnail ? (
					<span
						className="rundown-item-thumb"
						style={{
							display: "block",
							overflow: "hidden",
							background: "#000",
					...
					>
						<GraphicPreviewRenderer
							elements={liveThumbnail.elements}
							canvasWidth={liveThumbnail.canvasWidth}
							canvasHeight={liveThumbnail.canvasHeight}
					...
					<span className="rundown-item-thumb-placeholder">
						...
						{item.source_type === "ograf" && <Palette size={16} />}
					</span>
				)}
				<span className="rundown-item-info">
					<span className="rundown-item-type">{item.source_type}</span>
					<span className="rundown-item-name">{item.source_name}</span>
				</span>
				<span
					className={`rundown-quality-dot rundown-quality-dot--${qualityStatus}`}
					title={
						qualityIssueCount > 0
							? `송출 체크 이슈 ${qualityIssueCount}개`
							: "송출 체크 통과"
					}
				>
					{qualityIssueCount > 0 ? qualityIssueCount : ""}
				</span>
				{hasResolutionWarning && (
					<span
	...
	return (
		<main className="rundown-panel">
			<div className="rundown-header">
				...
				<span
					className={`rundown-quality-chip rundown-quality-chip--${qualitySummary.status}`}
				>
					{qualitySummary.errorCount > 0
						? `${qualitySummary.errorCount} 차단`
						: qualitySummary.warningCount > 0
							? `${qualitySummary.warningCount} 경고`
							: "체크 통과"}
				</span>
			</div>
						...
						>
							{sections.map((section) => {
								...
								return (
									...
									>
										{/* biome-ignore lint/a11y/noStaticElementInteractions lint/a11y/useKeyWithClickEvents: section header contains inline rename and delete controls. */}
										<div
										...
										<div
											className={`section-items ${isCollapsed ? "collapsed" : ""}`}
										>
												...
												(() => {
													...
													while (idx < sectionItems.length) {
														...
														if (item.track_layer === "wrap") {
															...
															rendered.push(
																<div
																	key={`wrap-group-${item.id}`}
																	className="rundown-tree-group"
																>
																	<div className="rundown-tree-parent">
																		<SortableRundownItem
																			...
																			hasResolutionWarning={checkResolutionWarning(
																				item,
																			)}
																			qualityStatus={
																				qualitySummary.itemStatusById[item.id]
																			}
																			qualityIssueCount={
																				qualitySummary.itemIssueCountById[
																					item.id
																				]
																			}
																			onSelect={() =>
																	...
																	{children.length > 0 && (
																		<div className="rundown-tree-children">
																			{children.map((child) => {
																				// ⚡ $O(1)$ 상수 시간 인덱스 맵 룩업 대조
																				const childGlobalIndex =
																					itemIndexMap.get(child.id) ?? 0;
																				return (
																					<SortableRundownItem
																						key={child.id}
																						item={child}
															...
															rendered.push(
																<SortableRundownItem
																	...
																	qualityIssueCount={
																		qualitySummary.itemIssueCountById[item.id]
																	}
							...
							{items.some((i) => !i.section_id) && (
								<div className="unsectioned-group">
									...
									<div className="section-items">
										{items
											.filter(
												(item) => !item.section_id && !item.parent_item_id,
											)
											.map((item) => {
												// ⚡ $O(1)$ 상수 시간 인덱스 맵 룩업 대조
												const globalIndex = itemIndexMap.get(item.id) ?? 0;
