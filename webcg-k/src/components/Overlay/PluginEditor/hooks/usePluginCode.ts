...
function isSameCode(a: PluginSourceCode, b: PluginSourceCode) {
	return (
		...
		a.css === b.css &&
		a.js === b.js &&
		JSON.stringify(a.motion ?? null) === JSON.stringify(b.motion ?? null)
	);
}

function trimHistory(history: PluginSourceCode[]) {
	return history.length > MAX_CODE_HISTORY
		? history.slice(history.length - MAX_CODE_HISTORY)
		: history;
}

function codeHistoryReducer(
	state: CodeHistoryState,
