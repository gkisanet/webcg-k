			...
			<div className="ml-auto flex items-center gap-4">
				...
				{hasSegments && (
					<button
						...
						title={
							autoFollow === "off"
								? "Auto-follow OFF — 클릭하여 Soft 모드로"
								: autoFollow === "soft"
									? "Soft-prompt — 힌트만 표시, 자동 전환 없음. 클릭하여 Auto로"
...
function TransitionLegend() {
	return (
		<div
			...
			title="Fade/Cut은 motion manifest가 없는 legacy 블록의 fallback 전환입니다. 패키지 모션 블록은 그래픽 내부 SHOW/HIDE를 사용합니다."
		>
...
export function TrackRow({
	...
	return (
		<div
			className={`track ${track.isLogoTrack ? "track-logo" : ""}`}
		...
		>
			...
			>
				...
				{/* 선택된 갭 표시 (줌 적용) */}
				{isGapSelected && <ZoomedGap gap={selectedGap} />}
				...
				{blocks.map((block) => {
					...
					return (
						<DraggableBlock
							...
							isDimmed={
								overlappingDuringDrag.includes(block.id) || isSegmentDimmed
							}
...
function ZoomedGap({
	...
	gap: { startPosition: number; endPosition: number };
}) {
	const zoom = useZoom();
	return (
		<div
...
export function RemotePlayhead({
	position,
	color,
	displayName,
	isScrubbing = false,
