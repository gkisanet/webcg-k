...
import type { RendererBroadcastPayload } from "../lib/playoutIntent";
import { sendRealtimeBroadcast } from "../lib/realtimeBroadcast";
import {
	isRendererGraphicCommandResultPayload,
	type RendererGraphicCommandResultPayload,
...
export type PlayoutPayload = RendererBroadcastPayload;
...
export function useSessionController(sessionId: string) {
	...
	const [graphicCommandResults, setGraphicCommandResults] = useState<
		Record<string, RendererGraphicCommandResultPayload>
	>({});
	...
	const pendingAcksRef = useRef<
		...
		>
	>(new Map());
	const heartbeatMonitorRef = useRef<ReturnType<
		typeof createHeartbeatMonitor
	> | null>(null);
	...
	const scheduleAckRetry = useCallback(
		(payload: PlayoutPayload, attempts = 0) => {
			...
			const timer = setTimeout(async () => {
				...
				if (attempts >= 2) {
					console.warn(
						`[SessionController] ACK timeout seq=${seqNum} — retry exhausted`,
					);
					clearPendingAck(seqNum);
					return;
				}
				...
				console.warn(
					`[SessionController] ACK timeout seq=${seqNum} — retry ${attempts + 1}/2`,
				);
		...
		dbChannel
			.on(
				...
				(payload) => {
					...
					if (updated) {
						console.log(
							"[SessionController] Session DB updated in realtime:",
							updated,
						);
						setSession(updated);
					}
	...
	useEffect(() => {
		...
		ch.on("broadcast", { event: "ack" }, (payload) => {
			...
			.on("broadcast", { event: "graphic-command-result" }, (payload) => {
				if (!isRendererGraphicCommandResultPayload(payload.payload)) return;
				const result = payload.payload;
				setGraphicCommandResults((current) => ({
					...current,
					[result.targetBlockId]: result,
				}));
			})
