...
| 분류 | 기술 | 용도 |
|------|------|------|
| **프레임워크** | React 19 + Vite + TanStack Start | SPA 렌더링 |
| **라우팅** | TanStack Router (File-based) | 페이지 내비게이션 |
| **상태 관리** | TanStack Store, Query, Table | 타임라인, 데이터 패칭, 테이블 |
| **스타일** | Vanilla CSS (Dark Mode + CSS Variables) | UI 테마 |
| **백엔드** | Supabase (PostgreSQL, Auth, Realtime, Storage) | DB/인증/실시간/파일 |
| **아이콘** | Lucide React | UI 아이콘 |
| **DnD** | @dnd-kit | 드래그앤드롭 |
| **AI** | Google Gemini 2.0 Flash | AI CG 그래픽 생성 |
| **타입 검증** | Zod | 런타임 스키마 검증 (as any 제거) |
| **다국어** | react-i18next | UI 텍스트 국제화 (10개 네임스페이스) |
...
└── docs/                               # 📖 프로젝트 문서
    ...
    └── guide/                         # 📖 워크플로우 가이드
        ...
        ├── NRCS_CUESHEET_WORKFLOW.md  # NRCS 큐시트 워크플로우
        ├── AI_CG_GUIDE.md             # AI CG 생성 가이드
        ├── AI_CHARACTER_SYSTEM.md     # AI 캐릭터 ViewModel 완전 가이드
        ├── RENDERER_RESOLUTION.md     # 렌더러 해상도 처리 가이드
        ├── REALTIME_SYNC_ARCHITECTURE.md # Realtime 동기화 아키텍처
        ├── SHADCN_GUIDE.md            # shadcn/ui + Tailwind v4 통합 가이드
        ├── GRID_EDITOR.md             # 그리드 편집기 기능 명세
        └── FONT_LICENSE_GUIDE.md      # 폰트 라이선스 정보
```
...
erDiagram
    ...
    profiles ||--o{ overlay_templates : owns
    ...
    broadcast_sessions ||--o{ session_action_logs : logs
    overlay_templates ||--o{ overlay_state : instantiates
    overlay_templates ||--o{ overlay_gallery : bookmarks
    ...
    broadcast_sessions {
        ...
        text title
        uuid rundown_id FK
        text status "idle|ready|live|ended"
        jsonb timeline_data
        jsonb current_pgm
        jsonb current_pvw
    }
    ...
    overlay_templates {
        ...
        text name
        int layer
        jsonb graphic_data "GraphicElement[]"
        jsonb animation_config
        jsonb data_source
        text ai_prompt "AI 생성 프롬프트"
    }
    ...
    overlay_state {
        ...
        uuid session_id FK
        uuid template_id FK
        boolean is_active "ON/OFF"
        text animation_state "idle|in|out"
        text conflict_mode "overlay|hide_block"
    }
...
> 상세 스키마: [DB.md](../supabase/docs/DB.md)
...
# 4. 환경 변수 설정
cp .env.example .env
# .env에 Supabase URL/Key 및 VITE_GEMINI_API_KEY 설정
...
| 문서 | 설명 |
|------|------|
| ⭐ **[USAGE.md](docs/USAGE.md)** | **통합 사용 메뉴얼** — 전체 워크플로우, 단축키, 단계별 가이드 |
| [SETUP.md](docs/guide/SETUP.md) | 환경 설정 가이드 |
| [TROUBLESHOOTING.md](docs/guide/TROUBLESHOOTING.md) | 트러블슈팅 |
| [DB.md](../supabase/docs/DB.md) | DB 스키마 상세 (ERD, RLS, 마이그레이션) |
...
| [GRID_EDITOR.md](docs/guide/GRID_EDITOR.md) | 그리드 편집기 기능 명세 |
...
| [AI_CHARACTER_SYSTEM.md](docs/guide/AI_CHARACTER_SYSTEM.md) | AI 캐릭터 시스템 |
| [RENDERER_RESOLUTION.md](docs/guide/RENDERER_RESOLUTION.md) | 렌더러 해상도 처리 |
| [SHADCN_GUIDE.md](docs/guide/SHADCN_GUIDE.md) | shadcn/ui + Tailwind v4 가이드 |
| [FONT_LICENSE_GUIDE.md](docs/guide/FONT_LICENSE_GUIDE.md) | 폰트 라이선스 정보 |
