...
graph TD
    ...
    subgraph "2. Data & Sync Layer"
        ...
        MigrationEngine["Single Consolidated Migration<br/>(86 files → 1, Fathom excluded)"]
    end
...
## 💾 Database Migration Consolidation Architecture
*   **Background**: Having 86+ fragmented migration files (including private Fathom analytics sub-system schemas) poses high risks of performance bottlenecks, dependency sequence errors, and schema pollution for open-source contributors.
*   **Design (Why & How)**:
    1.  A shell script (`merge_migrations.sh`) collects all SQL files in chronological order, filters out any file whose name contains `fathom` (case-insensitive, covering 4 private analytics tables: `fathom_archive`, `fathom_embedding`, `fathom_programs`, `fathom_cg_links`), and concatenates the remaining 82 files into `20260529000001_combined_migration.sql`.
    2.  Each source file's original filename is preserved as a `-- MIGRATION: <filename>` header comment block for traceability.
    3.  To prevent seeding failures in `seed.sql` caused by the empty search path (`pg_catalog.set_config('search_path', '', false)`) injected during `pg_dump`, `RESET search_path;` is appended at the end of the consolidated file.
*   **Result**: O(N) → O(1) I/O complexity for `supabase db reset`, with zero fathom schema references in the public repository.
