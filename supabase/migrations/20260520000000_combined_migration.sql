...
-- ■ api_keys.encrypted_key 컬럼 대칭 암호화 자동화 트리거
-- Why?: DB 디스크 저장(Rest) 시 평문 유출을 차단하기 위함. pgp_sym_encrypt를 사용해 대칭 암호화 수행.
...
BEGIN
  ...
  -- 이미 pgp_sym_encrypt로 base64로 포장된 암호화 텍스트가 아닌 경우에만 신규 암호화 집행
  IF NEW.encrypted_key IS NOT NULL AND NEW.encrypted_key != '' AND NOT (NEW.encrypted_key LIKE 'WYj%') THEN
...
-- ■ 안전한 키 조회를 위한 복호화 전용 보안 함수 정의
-- Why?: SELECT 시 평문 유출을 막고, 오직 인가된 런타임 서비스 레이어에서만 선택적으로 평문을 확보할 수 있게 래핑.
...
-- STUB: 리턴 구문 덮어쓰기 위해 실체 선언 분할 정의
CREATE OR REPLACE FUNCTION get_decrypted_api_key(key_id UUID)
RETURNS TEXT AS $$
DECLARE
  vault_key TEXT;
...
BEGIN
  ...
  -- 2단계: 이미 pgp_sym_encrypt로 base64 포장된 암호화 텍스트가 아닌 경우에만 신규 암호화 집행
  --        'WYj'는 pgp_sym_encrypt + base64 출력의 고정 헤더 prefix
  IF NEW.encrypted_key IS NOT NULL AND NEW.encrypted_key != '' AND NOT (NEW.encrypted_key LIKE 'WYj%') THEN
    NEW.encrypted_key := encode(pgp_sym_encrypt(NEW.encrypted_key, vault_key), 'base64');
  END IF;
